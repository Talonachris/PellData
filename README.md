...
2. Drop it into the **`/plugins`** folder.  
3. *(Important!)* Install [PlaceholderAPI](https://www.spigotmc.org/resources/placeholderapi.6245/).  
4. Start the server – PellData will create its files (including `custom.yml`).
...
### 🌐 Localisation
English file `en_us.yml` is loaded by default. You can also use the included `de_de.yml` or use `custom.yml` to create your own translation.
...
2. In den **`/plugins`**‑Ordner legen.  
3. *(Wichtig!)* [PlaceholderAPI](https://www.spigotmc.org/resources/placeholderapi.6245/) installieren.  
4. Server starten – PellData erstellt automatisch alle Dateien (inkl. `custom.yml`).
...
### 🌐 Lokalisierung
Die englische Datei `en_us.yml` wird standardmäßig geladen. Du kannst auch die mitgelieferte `de_de.yml` verwenden oder nutze `custom.yml`, um deine eigene Übersetzung zu definieren.
